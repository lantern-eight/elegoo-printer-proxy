...
async def detect_printer_type(ip: str, probe_timeout: float = 3.0) -> str:
  ...
  cc2_response, cc1_response = await asyncio.gather(
    _probe_udp(ip, _CC2_DISCOVERY_PORT, _CC2_DISCOVERY_MSG, probe_timeout),
    _probe_udp(ip, _CC1_DISCOVERY_PORT, _CC1_DISCOVERY_MSG, probe_timeout),
  )

  if cc2_response is not None:
    logger.info('CC2 discovery response from %s on port %d', ip, _CC2_DISCOVERY_PORT)
    return 'cc2'

  if cc1_response is not None:
